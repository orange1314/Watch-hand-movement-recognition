...
# GMM 相關函數
def compute_boundary_density_gmm(model_path, confidence_level=0.95, smooth=1, plot=True):
    """ 計算高斯混合模型 (GMM) 的邊界密度 """
...
# 指數分布相關函數
def compute_boundary_density_exp(model_path, confidence_level=0.95, smooth=1, plot=True):
    """ 計算指數分布的邊界密度 """
...
def compute_threshold_for_models(models_dir, confidence_level=0.95, smooth=1, plot=True):
    ...
    for variable in variables:
        ...
        mean_boundary_density = compute_boundary_density_gmm(model_path, confidence_level, smooth, plot)
